...
clean_2d5M <- function(f, dataDir, resultDir, bTestMode){
    load(paste0(dataDir, '\\adj_ppv_samp_', f,'.RData')) #adj_ppv_samp
    #remove likely HAE paitnes 
    ptid <- read.csv("F:\\Jie\\Shire_follow_up\\03_Results\\patient_id_9692.csv")[, 1]
    pt_rm_flag <- adj_ppv_samp$PATIENT_ID %in% ptid
    adj_ppv_samp <- adj_ppv_samp[!pt_rm_flag, ]
    ...
    data <- adj_ppv_samp %>%
        ...
        select(-matches('GENDERU|GENDERM|PATIENT_ID|REGION'))%>%
        #TEST     
        {
            if (bTestMode)
            {
...
#                         one_of(vars2Cap))
        load(paste0(resultDir, "capping_vct_onPos", '.RData'))
        ids <- match(vars2Cap, names(capping_vct_onPos)[!is.na(capping_vct_onPos)])
        ids <- ids[!is.na(ids)]
        capping_vct_onPos_match <- capping_vct_onPos[!is.na(capping_vct_onPos)][ids]
        temp_df <- as.data.frame(t(data.frame(capping_vct_onPos_match, capping_vct_onPos_match)))
        temp <- bind_rows(dataLastStep[,], temp_df[1, ])
        
        dataMutated <- 
            mutate_each(temp, funs(CapPercentile3(.)), 
                        one_of(vars2Cap))[-nrow(temp), ]
...
#     select(-LOOKBACK_DAYS)
    ...
    rm(adj_ppv_samp, data, data1)
  #  data$HAE <- 0
    save(result, file=paste0(resultDir, 'test2d5M_split', f,'.RData'))
}
...
sfExport('clean_2d5M', 'BucketAge', 'CapPercentile3')
...
ms_fromCV <- sfClusterApplyLB(1:n.split, clean_2d5M
                              , dataDir="F:\\Lichao\\work\\Projects\\HAE\\data\\from Dong\\Rep_Samp"
                              , resultDir="F:\\Jie\\Shire_follow_up\\03_Results\\for200+a\\"
                              , bTestMode=F)
sfStop()
...
{
    load(paste0(resultDir, 'test2d5M_split', f,'.RData')) #result
    names(result)[grep("AGE", names(result))] <- c("X30..AGE..49" ,  "X50..AGE..69",   "AGE..29"  ,     "AGE..70" )
    vars2norm <- intersect(names(result), as.vector(metrics_avg[, 1]))
    rownames(metrics_avg) <- metrics_avg[, 1]
    metrics_avg_t <- as.data.frame(t(metrics_avg[, -1]))
    data_bind <- bind_rows(metrics_avg_t, result)[, vars2norm] #[1] 37702   244
    rm(result)
    norm <- function(vct){
        min <- vct[1]
        range <- vct[2]
    ...
    temp <- mutate_each(data_bind, funs(norm(.))
                , one_of(vars2norm))[-(1:2), ]
...
#     names(data2) <- var_list
    save(temp, file=paste0(resultDir, 'test2d5M_norm_split', f,'.RData'))
    return(temp)
...
result <- sfClusterApplyLB(1:n.split, norm_2d5m
                           , resultDir="F:\\Jie\\Shire_follow_up\\03_Results\\for200+a\\")
sfStop()
...
pred_2d5m_par <- function(f, resultDir, sim_sel, norm, wt)
{
    ...
    data_matrix0 <- model.matrix(HAE~.
                                             , data=temp)[,-1]     # removes intercept term
    rm(temp)
    load(paste0(modelDir,"//total_model_simu", sim_sel, ".RData"))
    #data_matrix <- cbind(REGIONMIDWEST=0, data_matrix0)
    if(sim_sel==4){
        optN <- optN-1
    }
    pred<- predict(total_model, data_matrix0, type="response")[,optN]
    save(pred, file=paste0(modelDir, "//pred_2d5m_split", f, '_model', sim_sel,'.RData'))
    rm(data_matrix)
...
#parallel on n.folds cv
sfInit(parallel=TRUE, cpus=30, type='SOCK')
#sfSource("F:\\Jie\\Shire\\03_code\\subFunc_v3.R")
#sfSource("F:\\Jie\\Shire_follow_up\\02_Code\\clean_split_featureSelection_lasso.R")
...
sfClusterEval(library("dplyr"))
#cat(file=traceFile, append=TRUE, 'simulation:', i, ' parallele start running!\n')
 #randomly select the files to be applied to each model
split_rand <- sample(1:n.split)
result_nn1 <- sfClusterApplyLB(split_rand[1:10], pred_2d5m_par, resultDir, sim_sel=1, F, 1/200)
result_nn2 <- sfClusterApplyLB(split_rand[11:20], pred_2d5m_par, resultDir, sim_sel=2, F, 1/200)
result_nn3 <- sfClusterApplyLB(split_rand[21:30], pred_2d5m_par, resultDir, sim_sel=3, F, 1/200)
result_nn4 <- sfClusterApplyLB(split_rand[31:40], pred_2d5m_par, resultDir, sim_sel=4, F, 1/200)
result_nn5 <- sfClusterApplyLB(split_rand[41:51], pred_2d5m_par, resultDir, sim_sel=5, F, 1/200)

result_nm1 <- sfClusterApplyLB(split_rand[1:10], pred_2d5m_par, resultDir, sim_sel=1, T, 1/200)
result_nm2 <- sfClusterApplyLB(split_rand[11:20], pred_2d5m_par, resultDir, sim_sel=2, T, 1/200)
result_nm3 <- sfClusterApplyLB(split_rand[21:30], pred_2d5m_par, resultDir, sim_sel=3, T, 1/200)
result_nm4 <- sfClusterApplyLB(split_rand[31:40], pred_2d5m_par, resultDir, sim_sel=4, T, 1/200)
result_nm5 <- sfClusterApplyLB(split_rand[41:51], pred_2d5m_par, resultDir, sim_sel=5, T, 1/200)

sfStop()

fetch_pred_1kPos <- function(resultDir, norm, wt){
    temp <- lapply(1:n.simu, function(i){
        ...
        pred <- pred_ts_allSim[[i]]
        pred_pos <- pred[resp==1] #~200
        
        return(pred_pos)
    })  
    pred_pos <- unlist(temp)
    return(pred_pos)
...
result_perf <- summarize_perf(resultDir, n.simu=5, target_recall, wt, norm)
summarize_perf_2d5m <- function( resultDir, norm, wt, target_recall)
{
    modelDir <- paste0(resultDir, 'Lasso//wt=', wt, '_norm=', norm)
    load(paste0(modelDir, '//pred_allSim.RData')) #pred_ts_allSim
    pred_pos <- fetch_pred_1kPos(resultDir, norm, wt)
    pred_files_list <- grep("pred_2d5m_split", dir(modelDir), value = T)
    temp <- lapply(pred_files_list, function(f){
        load(paste0(modelDir, "//", f)) #pred
        return(pred)
    })
...
perf_nm <- summarize_perf_2d5m(resultDir, norm=T, wt=1/200, target_recall=target_recall)
#write.csv(perf_nm$ms, paste0(resultDir, 'Lasso//ms_on200+a_avg_wt=', wt, '_norm=', TRUE, ' 2d5m.csv'))
perf_nn <- summarize_perf_2d5m(resultDir, norm=F, wt=1/200, target_recall=target_recall)
write.csv(result_perf, paste0(resultDir, 'Lasso//ms_on200+a_avg_wt=', wt, '_norm=', FALSE, '.csv'))
...
perf_2d5m <- as.data.frame(cbind(result_perf, perf_nn$ms))
names(perf_2d5m) <- c("200K_withoutNorm", '2.5M_withoutNorm')
write.csv(perf_2d5m, paste0(resultDir, 'Lasso\\performance_on200K_2.5M_withoutNorm.csv'), row.names=T)

#get the coefficient of lookback_days
get_coef <- function(n.simu, resultDir, wt, norm){
    modelDir <- paste0(resultDir, 'Lasso//wt=', wt, '_norm=', norm)
    
    temp <-lapply(1:n.simu, function(i){
        #save(temp, file=paste0(resultDir, 'test2d5M_norm_split', f,'.RData'))
        load(paste0(modelDir,"//total_model_simu", i, ".RData"))
        model_coef<- total_model$beta[,optN]
        odds_ratio<- exp(model_coef)[model_coef != 0]
        non_zero_var<- names(model_coef)[model_coef != 0]
        coef <- data.frame(covar=non_zero_var, coef=model_coef[model_coef!=0], odds_ratio=odds_ratio)
        write.csv(coef, paste0(modelDir, '//coef_', 'norm=', norm, '.csv'), row.names=F)
        coef_1 <- coef[match('LOOKBACK_DAYS', non_zero_var), ]
        return(coef_1)
        
    })
    #return(temp)
    temp1 <- ldply(temp, rbind)
    #save(temp, file=paste0(modelDir, '//coef.RData'))
    write.csv(temp1, paste0(modelDir, '//coef_', 'norm=', norm, '_lookbackdays.csv'), row.names=F)
    
    return(temp1)
    
}
coef_result <- get_coef(n.simu=5, resultDir = "F:\\Jie\\Shire_follow_up\\03_Results\\for200+\\"
                            , wt=1/200
                        , norm=F)
