...
run;
proc sql;
select count(*)-152514686 as cnt_lhaein from outdir.final_data_pt_95M_&type._1;
quit;

/*exclude the Likely HAE patients*/
proc sql;
create table outdir.final_data_pt_95M_&type._2 as
select 
...
/*152514686
*/
/*exclude the 200K nonhae*/
proc sql;
create table outdir.final_data_pt_95M_&type. as
select 
	a.*, b.patient_id as flag_in_200K
from 
	outdir.final_data_pt_95M_&type._2 a left join outdir.nonhae_200k b
on 
	a.patient_id=b.patient_id
where
	flag_in_200k = .;
...
*/
/*proc sql;*/
/*select count(*) from outdir.final_data_pt_95M_&type. where age < 0;*/
/*quit;*/
...
/*	set outdir.final_data_pt_95M_&type.(obs=10);*/
	set outdir.final_data_pt_sample;
	if 1=2;
run;
...
%do i = 1 %to &cnt.;
	...
	%if &cnt_in3m. > 0 %then %do;
		data temp;
			set outdir.temp&i.;
			if lookback_days ge %eval(&lookback_days.- 90) and lookback_days le %eval(&lookback_days.+ 90);
		run;

		proc surveyselect noprint data=temp
					out=outdir.final_data_pt_sample method=srs seed=1234 sampsize = &cnt_in3m.;
		run;
	
		%let j = %eval(&i.+1);
		proc sql;
			create table outdir.temp&j. as 
				select a.*,
						b.patient_id as match_patient_id
				from outdir.temp&i. a left join outdir.final_data_pt_sample b
				on a.patient_id = b.patient_id;
		quit;
		data outdir.temp&j.;
			set outdir.temp&j.;
			match_patient_id = .;
			drop match_patient_id;
		run;
...
proc export data=outdir.nonhae_3M_index_95M_&type.
outfile="F:\shire_followup\data\200K_2.3M\95M_I1_I2_I3_I4(event=0)\nonhae_3M_index_95M_&type._May19.csv"
dbms=csv replace;
run;
