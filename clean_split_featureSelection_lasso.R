...
Clean_addPtId <- function(arglist)
{
    bTestMode <- arglist$bTestMode
...
#         select(-PATIENT_ID) %>%
    ...
    print("Saving the result..")
    write.table(result, sep=",", 
                file=paste(resultDir, "CleanDataB4Spliting_addPtId.csv", sep=""), 
                row.names=F)
...
#step3: for each simulation on training+validation do   
#a.	Cleanings / feature selection to perform inside the evaluation / simulation loop (i.e., the initial processing need to be performed on the training + validation data, then for the test data they need to be processed according to the conclusion from the training + validation data): 
