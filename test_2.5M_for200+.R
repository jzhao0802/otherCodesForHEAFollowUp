...
clean_2d5M <- function(f, dataDir, resultDir, bTestMode){
    ...
    data <- adj_ppv_samp %>%
        ...
        select(-matches('GENDERU|GENDERM|PATIENT_ID'))%>%
        #TEST     
#         {
#             if (bTestMode)
#             {
...
norm_2d5m <- function(f, resultDir)
{
    ...
    vars2norm <- intersect(names(data), as.vector(metrics_avg[, 1]))
    rownames(metrics_avg) <- metrics_avg[, 1]
    metrics_avg_t <- as.data.frame(t(metrics_avg[, -1]))
    data_bind <- bind_rows(metrics_avg_t, data)[, vars2norm] #[1] 37702   244
    rm(data)
    norm <- function(vct){
        min <- vct[1]
        range <- vct[2]
